...
if(!["818890015199068211"].some(role => message.member.roles.cache.get(role)) && (!message.member.hasPermission("ADMINISTRATOR"))) 
...
if (!ses) {
message.channel.send(new MessageEmbed().setColor('0x800d0d').setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setDescription("**<@"+ramo.id+"> Bir Sesli Kanalda Değil <a:aspendos_arp:818900823634346036>**"));
}
if (ses) {
...
"**<@"+ramo.id+"> İsimli Kişi `"+ses.name+"` İsimli Kanalda! <a:aspendos_ytik:818900828920348712>**"
));
}};
exports.conf = {
    enabled: true,
