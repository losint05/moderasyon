...
////----------------------- READY KISMI -----------------------\\\\
client.on('ready', () => {
    client.user.setPresence({ activity: { name: 'Aspendos ❤️ Losint' }, status: 'online' })
    client.channels.cache.get('818890016298237983').join() // ses kanalı İD
    client.channels.cache.get('818890016298237983').join() // ses kanalı İD
...
db.delete(`cezali_${member.guild.id + member.id}`)
    ...
    member.roles.remove('818890015207194678');
  }, ms(sürejail));
}
})
...
db.delete(`muteli_${member.guild.id + member.id}`)
    ...
    member.roles.remove('818890015207194675');
  }, ms(süre));
}
})
...
client.on("guildMemberAdd", member => {
  member.roles.add('819999066809303121'); // UNREGİSTER ROLÜNÜN İDSİNİ GİRİN
});
